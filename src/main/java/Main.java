...
public class Main extends JFrame {
    ...
    public Main() {
        ...
        styleButton(calcBinom, new Color(46, 204, 113)); //grün für Binome

        calcNormal = new JButton("Normales Rechnen");
        styleButton(calcNormal, new Color(52, 152, 219)); //blau für normales rechnen

        solveBtn = new JButton("Gleichung lösen");
        styleButton(solveBtn, new Color(155, 89, 182)); // Lila für Gleichungen
    ...
    private void starteGleichung() {
        try {
            ...
            // Gleichung: L_var + L_const = R_var + R_const
            // Ziel: (L_var - R_var) = (R_const - L_const)
            String varName = "";
    ...
    static class Parser {
        ...
        Polynomial parse(){ Polynomial x = sum(); return x; }
        Polynomial sum(){ Polynomial x = prod(); for(;;){ if(eat('+')) x=x.add(prod()); else if(eat('-')) x=x.sub(prod()); else return x; } }
        Polynomial prod() {
            Polynomial x = fact();
            for (;;) {
                if (eat('*')) {
                    ...
                    // nur Division durch Zahl erlauben
                    double val = d.terms.getOrDefault("", 0.0);

                    Polynomial r = new Polynomial();
                    for (var e : x.terms.entrySet()) {
                        r.addTerm(e.getKey(), e.getValue() / val);
                    }
                    x = r;
                }
                else return x;
